...
/**
 * openWakeWord three-stage pipeline (melspec → embedding → per-word classifier),
 * extended to run multiple wake-word classifiers in parallel over the shared
 * embedding stream. Apache-2.0 models bundled as assets.
 *
 * Usage:
 *   OpenWakeWordDetector(ctx, listOf("hey_jarvis_v0.1.onnx", "hey_mycroft_v0.1.onnx"))
 *     .process(chunk)  // -> List<Float> of scores per model
 */
class OpenWakeWordDetector(
    context: Context,
    wakeWordAssetFiles: List<String>,
) : AutoCloseable {

    private val env: OrtEnvironment = OrtEnvironment.getEnvironment()
    private val melSession: OrtSession
    private val embedSession: OrtSession
    private val wordSessions: List<OrtSession>

    private val audioBuffer = FloatArray(SAMPLE_RATE * AUDIO_WINDOW_SEC)
    ...
    init {
        ...
        wordSessions = wakeWordAssetFiles.map { loadFromAssets(context, "wakeword/$it") }
    }
    ...
    /**
     * Feed one 80 ms chunk (1280 int16 samples). Returns scores for each registered
     * wake-word model (same order as constructor), or null while the pipeline is
     * still filling its buffers.
     */
    fun process(chunkInt16: ShortArray): FloatArray? {
        require(chunkInt16.size == CHUNK_SAMPLES) {
            "Expected $CHUNK_SAMPLES samples, got ${chunkInt16.size}"
        }

        appendAudio(chunkInt16)
        if (audioBufferCursor < SAMPLE_RATE * MEL_MIN_WINDOW_SEC) return null
        ...
        val melFrames = runMelspec(input)
        ...
        return FloatArray(wordSessions.size) { idx -> runWakeWord(wordSessions[idx], embeddingBuffer) }
    }
    ...
    private fun runEmbedding(mel: Array<FloatArray>): FloatArray {
        val flat = FloatArray(MEL_WINDOW_FRAMES * MEL_BANDS)
    ...
    private fun runWakeWord(session: OrtSession, embeds: Array<FloatArray>): Float {
        ...
        OnnxTensor.createTensor(env, buf, shape).use { input ->
            session.run(mapOf("x.1" to input)).use { result ->
                @Suppress("UNCHECKED_CAST")
    ...
    companion object {
        ...
        const val EMBED_WINDOW = 16
        const val AUDIO_WINDOW_SEC = 4
        const val MEL_MIN_WINDOW_SEC = 2
        const val MEL_RUN_WINDOW_SEC = 2
...
private inline fun <T : AutoCloseable?, R> T.use(block: (T) -> R): R {
    try { return block(this) } finally { try { this?.close() } catch (_: Throwable) {} }
}
